          ...
          <v-row>
            <v-col cols="10" class="py-2">
              <v-card-text class="py-1">
                {{item.BatchId}}-#{{ item.CaldronNum }}-{{unit}}-{{currentProcess}}
                <br>是否变更釜次：否 使用不合格物料：无
                <!--筛选器-->
              </v-card-text>
            </v-col>
            <v-col cols="2">
                ...
                >
                  <v-row>
                    <v-col cols="3" class="py-0 pl-1">
                      <!--                      已完成-->
                      <v-icon small v-if="item.Status === 0" :color="item.Unusual === 1?'#FF5252':''">
                        mdi-checkbox-marked
                      </v-icon>
                      <!--                      正在-->
                      <v-icon small v-else-if="item.Status === 2" :color="item.Unusual === 1?'#FF5252':''">
                        mdi-checkbox-blank-outline
                      </v-icon>
                      <!--                      未开始-->
                      <v-icon small v-else-if="item.Status === 1" :color="item.Unusual === 1?'#FF5252':''">
                        mdi-checkbox-blank-circle
                      </v-icon>
                      <!--                      别人-->
                      <v-icon small v-else-if="item.Status === 3" :color="item.Unusual === 1?'#FF5252':''">
      ...
      submit(id, index) {
        ...
        if (lst.length > 0) {
          ...
          if (this.isCalculate) {                          //   计算公式一次提交数据
            ...
            // this.texts[index].Status = 0;
            // if (this.texts[index + 1]) {
            //   this.currentAction = this.texts[index + 1].Id;
            //   this.texts[index + 1].Status = 2
            // } else {
            //   for (let a of this.details) {
            //     if (a.Id === this.currentMenu) {
            //       a.Status = 1
            //     }
            //   }
            // }
